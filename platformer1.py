...
class platforms:
    ...
    def collisons(self):
        ...
        if not p_mask.overlap(platform_mask, (platform_rect.x - p.x, platform_rect.y - p.y)):
            gravity_var = True
        if p_mask.overlap(platform_mask, (platform_rect.x - p.x, platform_rect.y - p.y)):
            p.y -= 1
            gravity_var = False

        if p_mask.overlap(platform_mask, ((platform_rect.x - 1) - p.x, (platform_rect.y - 1) - p.y)):
            gravity_var = False
            jump_count = 2
            jump_var = True
...
platform2 = platforms(200, 200, "platform_1.png")
platform3 = platforms(350, 400, "platform_1.png")
platform4 = platforms(500, 450, "platform_1.png")
